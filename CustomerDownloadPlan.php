...
<table class="customers">
    ...
    $cid=$_SESSION['cid'];
      ...
      while($row=mysqli_fetch_array($r))
      {
        ...
        echo' <td><a href="Addpayment.php?id='.$row['customid'].'">Pay to view plan</a></td>';
        
        echo '</tr>';
      
      }

      $s="SELECT * FROM staff_custom_plan s,`customizedplan`c,`staff_reg` p WHERE  s.`customid`=c.`customid` AND s.`sid`=p.`sid` AND c.status = 'ReUploaded' AND c.cid ='$cid'";
      $r=mysqli_query($conn,$s);
      while($row=mysqli_fetch_array($r))
      {
        echo '<tr>';
